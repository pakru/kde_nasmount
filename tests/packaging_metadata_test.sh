...
# An upgrade must never tear anything down: prerm reaches its teardown from
# `remove)` only, so it must not handle `upgrade)` at all.
if grep -Eq '^\s*upgrade\)' "$repo_root/packaging/debian/nasmount.prerm.in"; then
    echo "ERROR: prerm handles upgrade; teardown must be reachable from remove) only" >&2
    exit 1
fi
...
# Both families tear managed shares down themselves rather than refusing
...
# The DEB does not refuse removal: prerm tears managed shares down itself and
# postrm purges their state. The guard must not appear in the DEB prerm, where
# it would refuse the removal this design performs.
