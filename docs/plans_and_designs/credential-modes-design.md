...
and authentication kind, plus an optional access-mode field. The pair must
agree, access included. The unit body is validated against a restricted
template; unknown functional directives, unsafe files, drop-ins, symlinks,
marker disagreement, and name/path disagreement are refused.

`X-Nasmount-Access` is the one optional field: it is omitted on write when the
value is read-write, and an absent field is read as read-write. That is what
keeps a read-write share's units byte-identical to what 0.1.0–0.1.3 wrote, so
adding the field did not turn every existing share into `Tampered`. Access
must live in the marker rather than in Store because validation re-derives
`Options=` from the marker alone; a mode only Store knew about would fail that
comparison.
...
bounded and reject control characters. Mount options are fixed per access
mode — the mode selects `ro` and the permission bits and nothing else; every
security-load-bearing term is identical in all three:

```text
read-write (the default, and what every release through 0.1.3 wrote)
...
read-write, executable files (for a Steam library and similar)
guest | credentials=<validated path>,nosuid,nodev,forceuid,forcegid,
uid=<caller>,gid=<caller>,nounix,iocharset=utf8,file_mode=0700,dir_mode=0700

read-only
guest | credentials=<validated path>,nosuid,nodev,forceuid,forcegid,
uid=<caller>,gid=<caller>,nounix,iocharset=utf8,ro,file_mode=0400,dir_mode=0500
```

`ro` and the permission bits are both required for read-only and neither is
redundant: `ro` sets `MS_RDONLY`, which the kernel enforces against every user
including root, while the modes are what let a file manager grey out
paste/rename up front instead of letting a copy fail partway. `dir_mode=0500`
rather than `0400` because a directory still needs `+x` to be traversable.

`rw` is never emitted. It would be semantically identical to emitting nothing
and would change the generated bytes for every existing share. The access mode
is chosen once, at Add; there is no in-place Edit.
